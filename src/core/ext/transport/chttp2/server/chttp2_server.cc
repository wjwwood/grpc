  ...
  if (!connection_->shutdown_ && result.ok() &&
    ...
    if (!is_callv3) {
      ...
      if (GPR_UNLIKELY(event_engine_endpoint == nullptr)) {
        ...
        // Use notify_on_receive_settings callback to enforce the
        // handshake deadline.
        transport = MakeRefCounted<http2::Http2ServerTransport>(
        ...
        if (GPR_LIKELY(channel_init_err.ok())) {
          connection_->state_ =
              DownCast<http2::Http2ServerTransport*>(transport.get())
...
absl::StatusOr<int> Chttp2ServerAddPort(Server* server, const char* addr,
  ...
  const bool is_callv3 = false;
#else
  const bool is_callv3 = IsPromiseBasedHttp2ServerTransportEnabled();
#endif
  ChannelArgs updated_args = args.Set(GRPC_ARG_USE_V3_STACK, is_callv3);
...
void grpc_server_add_channel_from_fd(grpc_server* server, int fd,
  ...
  if (!is_callv3) {
    ...
    if (event_engine_endpoint != nullptr) {
      ...
      error = core_server->SetupTransport(transport, nullptr, server_args);
      if (error.ok()) {
        static_cast<grpc_core::http2::Http2ServerTransport*>(transport)
            ->InitializeAndSpawnTransportLoops();
      }
    } else {
      LOG(ERROR) << "Failed to take endpoint.";
...
absl::Status grpc_server_add_passive_listener(
  ...
  const bool is_callv3 = grpc_core::IsPromiseBasedHttp2ServerTransportEnabled();
#endif
  args = args.Set(GRPC_ARG_USE_V3_STACK, is_callv3);
